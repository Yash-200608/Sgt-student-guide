...
@router.get("/api/timetable", include_in_schema=False)
...
@router.get("/api/timetable/{day}", include_in_schema=False)
async def get_timetable_by_day(day: str) -> dict[str, Any]:
    data = await timetable_service.get_timetable_entries_by_day(day)
    return success_response("Timetable entries retrieved successfully.", data)


@router.post("/api/timetable", status_code=status.HTTP_201_CREATED)
...
@router.delete("/api/timetable/{id}")
